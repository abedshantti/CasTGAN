...
from sklearn.preprocessing import MinMaxScaler, StandardScaler, LabelEncoder, OneHotEncoder, OrdinalEncoder, LabelBinarizer
...
#########################################################################################################################
    ...
    def _predict_proba_ordered(self, probs, classes_, all_classes):
        """
        probs: list of probabilities, output of predict_proba 
        classes_: clf.classes_
        all_classes: all possible classes (superset of classes_)
        """
        proba_ordered = np.zeros((probs.shape[0], all_classes.size),  dtype=np.float64)
    ...
    def tstr(self, train_set = None, test_set = None, categorical_cols = [], response_var = "", positive_val = "", pred_task = "binary_classification", syn_output = {}):
        ...
        if pred_task == "binary_classification" and (positive_val == None or positive_val == ""):
            # Auto-determine -- the positive var is set to be the minority target label
            positive_val = y_train.value_counts().index[1]

        if pred_task == "binary_classification":
            # Convert labels from cats to 0,1
        ...
        if pred_task ==  "binary_classification" or pred_task == "multi_classification":
            ...
            lgrg_pred = lgrg_clf.predict(X_test)

            train_acc_ada = accuracy_score(y_test, ada_pred)
            train_acc_rf = accuracy_score(y_test, rf_pred)
            train_acc_lgrg = accuracy_score(y_test, lgrg_pred)

            if pred_task == "binary_classification":
                ada_pred_proba = ada_clf.predict_proba(X_test)[:, 1]
                rf_pred_proba = rf_clf.predict_proba(X_test)[:, 1]
                lgrg_pred_proba = lgrg_clf.predict_proba(X_test)[:, 1]

                train_f1_ada = f1_score(y_test, ada_pred)
                train_f1_rf = f1_score(y_test, rf_pred)
                train_f1_lgrg = f1_score(y_test, lgrg_pred)

                train_roc_ada = roc_auc_score(y_test, ada_pred_proba)
                train_roc_rf = roc_auc_score(y_test, rf_pred_proba)
                train_roc_lgrg = roc_auc_score(y_test, lgrg_pred_proba)

                train_aucpr_ada = average_precision_score(y_test, ada_pred_proba)
                train_aucpr_rf = average_precision_score(y_test, rf_pred_proba)
                train_aucpr_lgrg = average_precision_score(y_test, lgrg_pred_proba)

            else:

                ada_pred_proba = ada_clf.predict_proba(X_test)
                rf_pred_proba = rf_clf.predict_proba(X_test)
                lgrg_pred_proba = lgrg_clf.predict_proba(X_test)

                binarizer = LabelBinarizer()
                binarizer.fit(y_test)
                y_test_binarized = binarizer.transform(y_test)

                ada_pred_proba_ordered = self._predict_proba_ordered(ada_pred_proba, ada_clf.classes_, binarizer.classes_)
                rf_pred_proba_ordered = self._predict_proba_ordered(rf_pred_proba, rf_clf.classes_, binarizer.classes_)
                lgrg_pred_proba_ordered = self._predict_proba_ordered(lgrg_pred_proba, lgrg_clf.classes_, binarizer.classes_)

                train_f1_ada = f1_score(y_test, ada_pred, average="weighted")
                train_f1_rf = f1_score(y_test, rf_pred, average="weighted")
                train_f1_lgrg = f1_score(y_test, lgrg_pred, average="weighted")

                train_roc_ada = roc_auc_score(y_test_binarized, ada_pred_proba_ordered, multi_class='ovr')
                train_roc_rf = roc_auc_score(y_test_binarized, rf_pred_proba_ordered, multi_class='ovr')
                train_roc_lgrg = roc_auc_score(y_test_binarized, lgrg_pred_proba_ordered, multi_class='ovr')

                train_aucpr_ada = average_precision_score(y_test_binarized, ada_pred_proba_ordered, average="weighted")
                train_aucpr_rf = average_precision_score(y_test_binarized, rf_pred_proba_ordered, average="weighted")
                train_aucpr_lgrg = average_precision_score(y_test_binarized, lgrg_pred_proba_ordered, average="weighted")
            

            avg_acc = np.mean([train_acc_ada, train_acc_rf, train_acc_lgrg])
            avg_f1 = np.mean([train_f1_ada, train_f1_rf, train_f1_lgrg])
            avg_roc = np.mean([train_roc_ada, train_roc_rf, train_roc_lgrg])
            avg_aucpr = np.mean([train_aucpr_ada, train_aucpr_rf, train_aucpr_lgrg])
